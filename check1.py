...
def is_dd_mm_yyyy_format(data):
    try:
        pd.to_datetime(data, format='%d-%m-%Y', errors='raise')
        return True
    except:
        return False
...
def generate_object_column_statistics(df):
    ...
    for col_name in categorical_columns.columns:
        ...
        # Check if the column contains data in 'dd-mm-yyyy' format (exclude date columns)
        if is_dd_mm_yyyy_format(col_data):
            continue
        ...
        for unique_value in unique_values:
            ...
            filtered_numeric_columns = filtered_df.select_dtypes(include=['int64', 'float64'])
            if filtered_numeric_columns.empty:
                continue

            statistics = filtered_numeric_columns.astype(float).describe().round(2)
            median_value = filtered_numeric_columns.astype(float).median().round(2)
            mode_value = filtered_numeric_columns.astype(float).mode().iloc[0].round(2)
            std_value = filtered_numeric_columns.astype(float).std().round(2)
            average_value = filtered_numeric_columns.astype(float).mean().round(2).squeeze()
...
def generate_summary_story(df):
    ...
    for col_name, unique_stats in object_column_statistics.items():
        for unique_value, stats in unique_stats.items():
            summary += f"Column '{col_name}' with unique value '{unique_value}' statistics:\n"
            summary += f"{stats['Statistics'].to_string()}\n"
            summary += "Averages:\n"
